...
const getAnimatedValues = ({ data = [] }) => {
  const animatedValues = {};

  data.forEach((_, index) => {
    animatedValues[index] = new Value(0);
  });
...
const Timeline = (props) => {
  ...
  const renderCircle = ({ item, index }) => {
    ...
    const lastActiveItemIndex = getLastActiveItemIndex();
    const lineWidthToUse = item.lineWidth || lineWidth || DEFAULT_LINE_WIDTH;
    const dotSizeToUse = item.dotSize || dotSize || circleSize * 0.8;
    ...
    if (innerCircleType === "dot") {
      const dotStyle = {
        height: dotSizeToUse,
        width: dotSizeToUse,
        borderRadius: dotSizeToUse / 2,
